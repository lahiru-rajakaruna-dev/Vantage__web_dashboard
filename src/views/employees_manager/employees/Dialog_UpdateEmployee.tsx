...
import { FaRegularCircleDot, FaSolidSort, FaSolidTrash } from 'solid-icons/fa';
...
export default function Dialog_UpdateEmployee(props: {
    ...
    const [ getSelectedEmployee, setSelectedEmployee ] = createSignal<Employee>()
    const [ getEmployeeUpdates, setEmployeeUpdates ]   = createSignal<Partial<Employee>>({})
    ...
    const mutationUpdateEmployee = useMutation(() => {
        return {
            mutationKey: [ 'employees', 'update', getSelectedEmployee() ],
            async mutationFn() {
                ...
                return await api.employeeApi.update(getEmployeeUpdates())
            },
            onMutate : async () => {
                props.setIsBusy(true)
                queryClient.setQueryData<string[], string[], Employee[]>([ 'employees' ], (employees) => {
                    ...
                    return employees.map((employee) => {
                        ...
                        return {} as Employee // FOR DEBUGGING
                    })
                })
            },
            onSettled: async () => {
                ...
                <Dialog.Content class={ 'w-full flex flex-col items-stretch justify-start bg-teal-200 border-2 border-teal-400 rounded-md' }>
                    ...
                    <div class={ 'p-4 flex flex-col items-stretch justify-start gap-2' }>
                        <Select
                                ...
                                options={ queryEmployees.data?.map((employee) => `${ employee.employee_id.slice(
                                                  0,
                                                  6
                                ...
                                onChange={ (employee_id__first_name) => {
                                    console.debug(employee_id__first_name)
                                    if (!employee_id__first_name) {
                                        return
                                    }
                                    
                                    if (!queryEmployees.data) {
                                        return
                                    }
                                    
                                    const selectedEmployee = queryEmployees.data.find((employee) => {
                                        return employee.employee_id.slice(0, 6) ===
                        ...
                        { getSelectedEmployee() ?
                          <div class={ 'p-2 flex flex-row flex-wrap gap-2 gap-y-4 border-2 border-teal-400/60 rounded-md' }>
                              {
                                  Object.entries(getSelectedEmployee() ?? {}).map((entry) => {
