...
export default function Table() {
    ...
    const columns = createMemo(() => [
        ...
        columnBuildHelper.accessor((row) => `${ row.employee_first_name ??
                                                'not-registered' } ${ row.employee_last_name ??
                                                                      'not-registered' }`, {
                                       id                : 'employee_name',
                                       header            : () => <pre>Name</pre>,
                                       sortingFn         : 'text',
                                       cell              : (row) =>
                                               <pre class={ 'px-4' }>{ row.getValue()
                                                                          .toString()
                                                                          .padEnd(
                                                                                  15,
                                                                                  ' '
        ...
        columnBuildHelper.accessor((row) => row.employee_active_territory, {
            ...
            enableSorting     : true,
            sortingFn         : 'text',
            cell              : (row) => <pre>{ row.getValue()?.toString() ??
                                                'not-set' }</pre>,
            enableResizing    : true,
            enableColumnFilter: true
        }),
    ...
    return <table class={ 'w-full h-full flex flex-col items-stretch justify-start overflow-visible' }>
        ...
        <For each={ getRowModel().rows }>
            {
                (row) => {
                    return <tr class={ 'px-2 flex flex-row items-stretch justify-start hover:bg-yellow-200/60 transition-all duration-300' }>
