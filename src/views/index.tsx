...
const tabs: Array<{ key: string, title: string, component: () => JSX.Element, icon: () => JSX.Element }> = [
    {
        key      : 'dashboard',
        title    : 'Dashboard',
                    ...
                    <Tabs.List class={ 'z-10 w-3/12 min-w-56 max-w-60  h-full flex flex-col items-stretch justify-start transform-3d perspective-normal perspective-origin-top-left backdrop-blur-sm border-r-2 border-r-v-bg/80 bg-v-accent/30' }>
                        <For
                            ...
                            { (item, index) => {
                                return <Tabs.Trigger
                                        value={ item.key }
                                        class={ `py-4 px-3 font-normal last:mt-auto origin-top-left transition-all ${ item.key ===
                                                                                                                      getSelectedTab() ? 'translate-z-3' : 'translate-none' } ${ item.key ===
                                                                                                                                                                                 getSelectedTab() ? 'shadow-md' : 'shadow-none' } ${ item.key ===
                                                                                                                                                                                                                                     getSelectedTab() ? 'bg-yellow-200/60' : 'bg-teal-200/60' } border-b-2 border-v-bg/30 last:border-0 backdrop-blur-sm` }
                                >
                                    <div class={ 'w-full h-full flex flex-row items-center justify-start gap-4' }>
                                        ...
                                        <div
                                                class={ `w-full h-full flex flex-col items-start justify-center whitespace-nowrap ${ item.key ===
                                                                                                                                     getSelectedTab() ? 'text-v-text-main font-normal' : 'text-v-text-body' } font-light` }>{ item.title }</div>
                                    </div>
                                </Tabs.Trigger>
                            } }
                        </For>
